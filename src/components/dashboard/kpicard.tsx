...
export function KpiCard({ label, value, delta, tone }: StatRecord) {
  return (
    <motion.div
      whileHover={{ y: -4, scale: 1.01 }}
      className="group relative overflow-hidden rounded-[28px] border border-white/10 bg-card/80 p-5 shadow-glass transition hover:border-neon/45 hover:shadow-neon"
    >
      ...
      <div className="mt-5 flex items-end justify-between">
        ...
        <div className={clsx('rounded-full border px-3 py-1 text-xs font-semibold', toneMap[tone], 'border-current/20 bg-white/5')}>
          {delta}
        </div>
      </div>
      <div className="mt-6 h-12 rounded-2xl border border-white/5 bg-gradient-to-r from-white/5 to-transparent" />
    </motion.div>
  )
}
